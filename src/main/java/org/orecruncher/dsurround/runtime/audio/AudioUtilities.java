...
public final class AudioUtilities {
    ...
    private static final Lazy<Boolean> advancedProcessingEnabled = new Lazy<>(() -> {
        ...
        if (config.enableEnhancedSounds) {
            ...
            for (var modId : autoDisabledBecauseOf) {
                if (PlatformCompat.isModLoaded(modId)) {
    ...
    // Number of auxiliary sends the OpenAL device supports. 26.1 fix: the reverb system
    // assumed 4 sends (send 0..3) but most devices only expose 2, causing AL_INVALID_VALUE
    // on sends 2/3 and silencing the long-reverb zones used for halls/caves.
    private static int MAX_AUX_SENDS = 0;

    public static int getMaxAuxSends() {
        return MAX_AUX_SENDS;
    }
    ...
    public static String debugString(final SoundInstance sound) {
        ...
        try {
            ...
            if (underlyingSound != null) {
                sb.append(String.format(", v: %.4f", sound.getVolume()));
                sb.append(String.format(", p: %.4f", sound.getPitch()));
            ...
            if (!sound.isRelative()) {
                ...
                var distance = Math.sqrt(listener.getListenerPosition().distanceToSqr(sound.getX(), sound.getY(), sound.getZ()));
    ...
    public static void initialize(final Library soundEngine) {
        ...
        try {
            ...
            // Capture the device's auxiliary send count BEFORE the effects system is
            // initialized so the reverb zones can be mapped onto the available sends.
            MAX_AUX_SENDS = ALC11.alcGetInteger(devicePointer, EXTEfx.ALC_MAX_AUXILIARY_SENDS);
            ...
            // DIAG(1.20.1): reverb chain diagnostics
            org.orecruncher.dsurround.lib.Library.LOGGER.debug("EFX_DIAG device=%d maxSounds=%d auxSends=%d enhanced=%s",
                    devicePointer, MAX_SOUNDS, MAX_AUX_SENDS, doEnhancedSounds());
            ...
            LOGGER.info("AuxSends: %d", MAX_AUX_SENDS);
    ...
    public static void deinitialize(final Library ignore) {
        // Always deinitialize: the sound system may be torn down for reasons other than
        // the enhanced-sounds toggle (config reload, device change, reverb/occlusion
        // toggle), and skipping this leaves stale OpenAL handles + a leaked worker.
