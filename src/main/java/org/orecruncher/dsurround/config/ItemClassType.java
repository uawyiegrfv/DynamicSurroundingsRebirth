...
public enum ItemClassType {
    ...
    ItemClassType(String name) {
        this.name = name;
        this.toolBarSound = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "toolbar." + name + ".equip");
        this.swingSound = Identifier.fromNamespaceAndPath(Constants.MOD_ID, "toolbar." + name + ".swing");
    }
    ...
    public @Nullable ISoundFactory getToolBarSound() {
        // The factory may not be loaded yet (e.g. before the first SoundLibrary reload),
        // so fall back to null instead of throwing.
        return SOUND_LIBRARY.getSoundFactory(this.toolBarSound).orElse(null);
    }
