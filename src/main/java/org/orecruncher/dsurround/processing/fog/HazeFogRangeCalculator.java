...
public class HazeFogRangeCalculator extends VanillaFogRangeCalculator {
    ...
    public FogRenderer.FogData render(@NotNull final FogRenderer.FogData data, float renderDistance, float partialTick) {
        ...
        bestEnd = Math.min(bestEnd, this.bandEnd(cloudBand, eyeY, data.end));
        bestEnd = Math.min(bestEnd, this.bandEnd(highBand, eyeY, data.end));
        ...
        result.start = bestEnd * 0.5F;
        return result;
    }

    /**
     * Fade the far plane from vanilla at the band edges to the capped distance in the
     * core, so entering/exiting the band isn't abrupt. Returns data.end
