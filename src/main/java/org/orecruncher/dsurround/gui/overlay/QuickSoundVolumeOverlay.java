...
public class QuickSoundVolumeOverlay implements SoundEventListener {
    ...
    private static final class RecentSound {
        ...
        RecentSound(final ResourceLocation id, final Component text, final float range, final Vec3 location) {
            this.id = id;
            this.text = text;
            this.range = range;
    ...
    private final Map<ResourceLocation, RecentSound> recent = new Object2ObjectOpenHashMap<>();
    ...
    public QuickSoundVolumeOverlay() {
        ...
        MinecraftForge.EVENT_BUS.addListener(this::onScroll);
    }
    ...
    @Override
    public void onPlaySound(final SoundInstance sound, final WeighedSoundEvents soundEvent) {
        ...
        if (existing != null) {
            ...
            if (this.recent.size() >= MAX_TRACKED)
                this.recent.values().stream().min(Comparator.comparingLong(r -> r.playedAt.isEmpty() ? 0L : r.playedAt.get(r.playedAt.size() - 1).time)).ifPresent(r -> this.recent.remove(r.id));
            // 1.20.1: SoundEventListener has no range parameter; approximate from the sound volume.
            float range = 16.0F + sound.getVolume() * 16.0F;
    ...
    private void tick(final Minecraft client) {
        ...
        final boolean up = InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_UP);
        final boolean down = InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_DOWN);
        if (up && !this.prevUp)
        ...
        final long now = Util.getMillis();
        final boolean left = InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_LEFT);
        final boolean right = InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_RIGHT);
    ...
    private static boolean isCtrlDown() {
        ...
        return InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_LCONTROL)
                || InputConstants.isKeyDown(window.getWindow(), InputConstants.KEY_RCONTROL);
    }
    ...
    private void openSnapshot(final Minecraft client) {
        ...
        this.recent.values().stream()
                ...
                .sorted(Comparator.comparingLong((RecentSound r) -> r.playedAt.get(r.playedAt.size() - 1).time).reversed())
                .limit(MAX_ENTRIES)
                .forEach(r -> {
    ...
    private static IndividualSoundConfigEntry findOrCreateEntry(final ResourceLocation id) {
     ...
     */
    public void render(final GuiGraphics graphics, final float partialTick) {
        if (!this.visible)
            return;
        ...
        for (final var line : titleLines) {
            graphics.drawString(font, line, panelX + PADDING, y, COLOR_TEXT, false);
            y += ROW_HEIGHT;
        }
        for (final var line : hintLines) {
            graphics.drawString(font, line, panelX + PADDING, y, COLOR_DIM, false);
            y += ROW_HEIGHT;
        }
        ...
        for (int i = 0; i < rows; i++) {
            ...
            // Selection marker
            graphics.drawString(font, selected ? ">" : " ", panelX + PADDING, y, COLOR_ACCENT, false);
            ...
            graphics.drawString(font, trimmed, nameX, y, COLOR_TEXT, false);
            ...
            graphics.drawString(font, label, panelX + PANEL_WIDTH - PADDING - font.width(label), y, color, false);
