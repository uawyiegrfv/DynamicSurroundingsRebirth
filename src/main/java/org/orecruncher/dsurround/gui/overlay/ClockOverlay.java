...
public class ClockOverlay extends AbstractOverlay {
    ...
    public void tick(Minecraft client) {
        ...
        if (this.config.compassAndClockOptions.enableClock && GameUtils.isInGame()) {
            ...
            var angleDegrees = (world.getDayTime() % 24000) * 360F / 24000F + 180;
            // Wrap
            if (angleDegrees >= 360)
                angleDegrees -= 360;
    ...
    public void render(GuiGraphics graphics, float partialTick) {
        ...
        TooltipRenderUtil.renderTooltipBackground(graphics, x - this.renderWidth / 2, y, this.renderWidth, this.renderHeight, 0);
        graphics.drawCenteredString(textRender, this.clockDisplay.get(0), x, y, this.color);
        if (this.clockDisplay.size() == 2)
            graphics.drawCenteredString(textRender, this.clockDisplay.get(1), x, y + textRender.lineHeight, this.color);
    }
}
