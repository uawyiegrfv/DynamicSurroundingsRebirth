 ...
 * ({@code tryBlendFuncSeparate(SRC_ALPHA, ONE, ONE, ZERO)}). 1.20.1 has no immediate mode, so the
 * same band geometry is submitted through {@code MultiBufferSource} using
 * {@code RenderType.debugQuads()}, which is POSITION_COLOR + QUADS +
 * <b>translucent</b> blend.
 *
 * <p>So this is a close approximation, NOT a faithful drop-in, and the difference is deliberate:
 * building a custom additive RenderType for a path that only runs when the shader pipeline failed to
 * register would add a render-state surface for a fallback nobody should ever see. Where it shows:
...
public final class AuroraClassic extends AuroraBase {
    ...
    private static void quad(final PoseStack poseStack, final VertexConsumer consumer,
        ...
        consumer.addVertex(pose, (float) x0, (float) y0, (float) z0)
                .setColor(base.red, base.green, base.blue, alpha);
        consumer.addVertex(pose, (float) x1, (float) y1, (float) z1)
                .setColor(fade.red, fade.green, fade.blue, 0.0F);
