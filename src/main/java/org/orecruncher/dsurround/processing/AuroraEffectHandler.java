 ...
 * 1.20.1 has no immediate mode, so rendering happens on the Forge
 * {@link RenderLevelStageEvent} AFTER_SKY stage, submitting POSITION_COLOR quads
 * (classic) or POSITION_TEX_COLOR quads (shader) through
...
public class AuroraEffectHandler extends AbstractClientHandler {
    ...
    public AuroraEffectHandler(Configuration config, Scanners scanners, IModLog logger) {
        ...
        MinecraftForge.EVENT_BUS.addListener(this::doRender);
    }
    ...
    public void doRender(final RenderLevelStageEvent event) {
        // 1.20.1: a single RenderLevelStageEvent with a stage field, not per-stage subclasses.
