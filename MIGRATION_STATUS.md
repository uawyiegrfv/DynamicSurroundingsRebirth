...
## 2.32 声音快捷调节浮层（按住 Ctrl+反引号）（2026-08-22，`d555f56`）

按住 **Ctrl+反引号（`）** 呼出"最近声音快捷调音量"浮层（`QuickSoundVolumeOverlay`），不必进设置界面即可逐条调整刚听到的各声音音量，实时生效。

- **条目来源**：`SoundEventListener` 跟踪字幕声音，打开浮层时按原版 SubtitleOverlay 语义**快照**（有字幕 + 可听度过滤 + 3000ms×通知时长停留窗口）。
- **非 Screen 实现**：不占屏，WASD/视角不受影响；上下键轮询切换（切换/打开即试听该条目），滚轮每格 1%（NeoForge `MouseScrollingEvent` 取消，不切快捷栏），左右键单击 1%、按住 400ms 后加速至每 tick 5%。
- **渲染**：左下角字幕行高处；名称 + 动态拉伸滑条 + 百分比（0% 显示"关"、>100% 金色提示）；750ms 防抖持久化到 `soundconfig.json`。
- **26.1 图层坑**：初始注册在淡入淡出层之下，夜间被四角暗 vignette 盖暗——后续（2.35）把注册层提升到 `VanillaGuiLayers.CAMERA_OVERLAYS` 之上才彻底可读；英文提示原溢出面板，加按宽度换行。

## 2.33 脚步声三项修正（2026-08-22，`9aefe18`）

- **playJump/playLand 改经工厂注册表播放**：音高随机与 JSON 音量配置重新生效。根因：此前裸用 `SoundFactoryBuilder` 绕过了工厂配置——跳跃恒定音高、调落地音量无效。
- **新增 13 个工厂条目**：`player.jump`（pitch 0.95–1.05）+ 12 个材质 `_wander` 变体（pitch 0.8–1.2，行走使用）。
- **轻型生物豁免**：新增实体标签 `effects/light_steps`（鸡/兔/猫等 14 种）——轻型生物保留原版脚步，不再替换为玩家材质声（避免小脚生物脚步声失真）。
- 玩家脚步基础音量 **0.55→0.45**，让落地相对更突出。

## 2.34 下界与末地环境音（2026-08-23，`c075f9d`）

移植 1.12.2 的维度氛围（`biomes.json` 选择器 + `BiomeScanner`）：

- **下界 5 群系** → `dimension.nether` 低鸣（素材/事件早已存在，缺的只是引用）。
- **末地** → `theend` 空灵循环（新注册，中英字幕同步）。
- **根因修复**："It's The End!" 规则（priority 100）移植时只带了 `clearSounds` 却**漏了 theend 声音列表**——导致末地群系声音先被加进、随后被该规则清空而从未出声。
- 灵魂沙低语（`soulsand.laughter`）已在工作，无需改动。

## 2.35 声音配置界面修复 + 极光亮度确认（2026-08-24，`4f132fa`）

"极光/音频会话"整体提交（19 文件），含声音配置界面 UX、极光亮度、枯叶落地声、浮层图层、深暗之域氛围增强：

- **声音配置界面 UX 修复**（`IndividualSoundControlScreen`）：
  - 完成/取消/**ESC** 统一返回父界面（音乐和声音界面）——根因：26.1 `onClose()` 走 vanilla `popGuiLayer()` 弹层栈，而 DS 界面是用 `setScreen()` 替换的（下层就是游戏层），ESC 一路弹出游戏；改为 `onClose` 走 `closeScreen()` 同一条路径。
  - **光标保持原位**：记录打开前鼠标绝对坐标，`setScreen` 返回后用 `GLFW.glfwSetCursorPos` 放回（vanilla 每次切屏都会把光标居中，此为全局行为，需显式回置）。
  - 完成/取消补**点击音效**（`SoundEvents.UI_BUTTON_CLICK`）。
- **极光亮度确认**：极光实际走 shader 版（`AuroraShader`，顶点色用 bandTints），此前 `auroraBrightness` 滑块挂在经典回退版 `getBaseColor`，对 shader 渲染无效——**撤销无效滑块**，亮度由 `aurora.fsh` 的 `BRIGHTNESS` 常量控制（值 **1.2**，此乃稳定值）。
- **枯叶落地声 6 变体**：删走路 3/旧落地变体，走路 2 降 3 半音作落地变体，三个落地音去底噪并统一 0.45s，每音各出一个变体（共 6 个）+ 走路变体。
- **快捷音量浮层**：标题/提示按面板宽度换行，注册层提升到 `CAMERA_OVERLAYS` 之上（夜间不再被四角暗 vignette 盖暗）。
- **深暗之域氛围增强**：`BiomeScanner` 让 `deep_dark` 解析为自身（恢复 drone/心跳/sculk 氛围），生物群系雾 `light`、基岩雾收窄至 `minY..minY+10`。
- 混响 `GLOBAL_REVERB_MULTIPLIER` 0.7→0.6；穿水音量默认 **0.95**、闷感默认 **0.7**。
- `.gitignore` 忽略本地音频试听文件夹。

## 2.36 版本 1.0.0 + README 功能清单 + 作者署名（2026-08-24）

- **版本 1.0.0**（`bcd3b83`）：`gradle.properties` 的 `mod_version` 0.5.0→1.0.0 正式发布。产物 `DynamicSurroundingsRebirth-26.1.2-1.0.0.jar`（另含 `-1.0.0-dev-shadow.jar` 开发用）。
- **作者署名改为 GitHub 账号名**（`e5f6a81`）：README 与 `gradle.properties` 的 `mod_author`/`mod_description` 由 `deepsleep114` 改为 `uawyiegrfv`，与 GitHub 远程账号一致。
- **README 功能清单补充**（`70cc4c6`/`c0db0c8`）：按玩家视角补穿水声学、脚印、玩家状态声、天气氛围、岩浆/灼热块、季节支持等；**移除夸大的天气/降水暗示**（天气系统仅简化粒子版）。
- 开发文档（`docs/`、`MIGRATION_STATUS.md`、`fix_26.1_api.py`）自 `bb949e0` 起从 git 解除跟踪，磁盘原样保留。

## 2.37 晨雾视距修复 + 季节/天气支持澄清（2026-08-24，`e5f6a81`）

- **晨雾受视距影响（bug，已修）**：原公式 `newEnd = renderDistanceEnd − (renderDistanceEnd − PEAK_FOG_END)×factor` 使雾墙距离随视距拉扯，视距 32 区块（≈1024 格）时 HEAVY 峰值也被推到 ~243 格、NORMAL ~565 格——"视距调高后雾在极远处，几乎看不到"，并掩盖了季节差异。修复（`MorningFogRangeCalculator`）：雾墙距离**仅由密度和时刻决定、与视距无关**——按密度给固定雾墙峰值（`peakEnd`：HEAVY 48/MEDIUM 64/NORMAL 96/LIGHT 160，上限 `MAX_FOG_VIEW_END=256`），取 `wallEnd = peakEnd + (MAX − peakEnd)×(1−strength)`，最后仅 clamp 到视距末端。
- **季节支持澄清**：晨雾 `getFogType()` 按 `seasonInfo.isSpring()/…` 抽密度表（春夏秋冬各有倾向），逻辑正确；但**需装 Serene Seasons 才走真实季节**（否则恒为"春"，无跨季节差异）。代码层面审计 SereneSeasons 集成完整（`ISeasonalInformation` 接口 + `SeasonManager` 按 `isModLoaded` 选择，26.1 API 签名经 javap 比对全部匹配：`SeasonHelper.getSeasonState`、`SeasonHooks.getBiomeTemperature/getPrecipitationAtSeasonal` 的 seaLevel 第 4 参数等），消费者（时钟/晨雾/霜息/天气温度/条件脚本/诊断）齐全——**仅未运行时实测**。
- **天气系统澄清**：确认只实现**简化粒子版**（`WeatherStormHandler`：沙漠黄雾沙尘 + 下界尘雨）；雨/雪的概率、大小、纹理替换（A17 完整版，`WeatherEffectRenderer` 重写）**未做**。
