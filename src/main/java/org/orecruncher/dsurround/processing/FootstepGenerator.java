...
public class FootstepGenerator extends AbstractClientHandler {
    ...
    private record LandComposition(Identifier primary, Identifier secondary, Identifier echo) {}
     ...
     */
    private static Optional<Identifier> resolveMaterial(final Player player) {
    ...
    private static BlockState resolveSurfaceBlock(Player player, Level level, BlockPos pos) {
        ...
        if (!footState.isAir() && footState.getFluidState().isEmpty()
                && !(footState.getBlock() instanceof net.minecraft.world.level.block.VegetationBlock)
     ...
     */
    private Identifier resolveLandSound(final Player player) {
        var state = resolveSurfaceBlock(player, player.level(), player.blockPosition().below());
        if (state.isAir() || !state.getFluidState().isEmpty())
            return LAND;
     ...
     */
    @Nullable
    private static Identifier materialVariant(Identifier material, String suffix) {
