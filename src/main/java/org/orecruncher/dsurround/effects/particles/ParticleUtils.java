...
public final class ParticleUtils {
    ...
    /**
     * Look up the SpriteSet registered for the given particle type. 26.1 moved the sprite
     * table out of ParticleEngine into ParticleResources; both are reached via accessors.
     */
    public static SpriteSet getSpriteProvider(ParticleType<?> particleType) {
        ...
        var resources = ((MixinParticleManager) GameUtils.getParticleManager()).dsurround_getResourceManager();
        return ((MixinParticleResources) resources).dsurround_getSpriteSets().get(id);
    }

    /**
     * Fetch a single sprite from the particle texture atlas. The ripple strip is registered
     * into the atlas via assets/minecraft/atlases/particles.json (a single-file source).
     */
    public static TextureAtlasSprite getSprite(Identifier id) {
        var texture = GameUtils.getTextureManager().getTexture(TextureAtlas.LOCATION_PARTICLES);
        if (texture instanceof TextureAtlas atlas)
            return atlas.getSprite(id);
        return ((TextureAtlas) texture).missingSprite();
    }
