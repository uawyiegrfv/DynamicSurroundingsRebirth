...
public final class SoundInstanceHandler {
     ...
     * @return True if the sound play is to be blocked outright, false otherwise
     */
     ...
     *
     * @param theSound The sound that is being played
     * @return True if the sound play is to be culled, false otherwise
     */
    public static boolean shouldCullSoundPlay(final SoundInstance theSound) {
        // Don't cull ConfigSoundInstances.
        if (theSound instanceof ConfigSoundInstance)
            return false;
    ...
    public static boolean outOfRange(final SoundInstance sound, final int pad) {
        // 1.20.1's Listener has no transform getter; the camera position is what
        // vanilla feeds the OpenAL listener each tick, so it is equivalent.
        var mc = Minecraft.getInstance();
        if (inRange(mc.gameRenderer.getMainCamera().getPosition(), sound, pad))
            return false;
