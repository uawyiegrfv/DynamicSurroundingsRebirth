    ...
    public void renderGui(GuiGraphics graphics, float partialTick) {
        ...
        for (var entry : this.active) {
            ...
            if (diag) {
                ...
                // test also fired when the scale went up, which is simply the growth phase.
            ...
            try {
                // Deliberately NOT snapped to whole GUI pixels. Pixel snapping was added while
                // chasing the "flashes bright for one frame" report on the theory that sub-pixel
                // origins changed glyph coverage. That was wrong - the flash was vanilla
                // Font.adjustColor forcing alpha < 4 to fully opaque (guard above). Snapping only
                // cost sub-pixel smoothness: at 300+ fps the origin advances less than a pixel per
                // frame, so rounding made the text sit still and then jump. Do not re-add it
                // without a demonstrated rasterisation artifact.
                graphics.pose().translate(sx, sy, 0.0F);
                graphics.pose().scale(textScale, textScale, 1.0F);
                // The shadow has to fade with the text (see the note in the 1.21.1 port): it used
                // to be drawn at a fixed opaque 0xFF000000 while the text faded, leaving a solid
                // black silhouette in the last frames.
