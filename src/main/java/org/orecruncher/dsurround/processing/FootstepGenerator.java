...
public class FootstepGenerator extends AbstractClientHandler {
    ...
    static final Map<String, LandComposition> LAND_COMPOSITIONS = Map.ofEntries(
            ...
            // a heavy run thud (not the walk "step") with a subtle walk layer and a fused run echo.
    ...
    private void playStep(final Player player, final boolean running) {
        ...
        // isLeafLitter is permanently false on 1.21.1 (LeafLitterBlock is 1.21.2+), so
        // this dedup block is inert here by design - see isLeafLitter further down.
        if (isLeafLitter(state)) {
    ...
    /**
     * 1.21.1 compat: LeafLitterBlock is a 1.21.2+ block, so no state can be one here
     * (flip this helper when backporting to 1.21.2+). VegetationBlock is also 1.21.4+;
     * the 1.20.1 lineage used BushBlock as its stand-in and 1.21.1 still has BushBlock
     * (sweet berry bush etc.), so the same check applies.
     */
    private static boolean isLeafLitter(net.minecraft.world.level.block.state.BlockState state) {
        return false;
    }
