...
public class CompassOverlay extends AbstractOverlay {
    ...
    public void render(GuiGraphicsExtractor graphics, float partialTick) {
        ...
        try {
            ...
            float x = (graphics.guiWidth() - BAND_WIDTH * this.scale) / 2F;
            float y = (graphics.guiHeight() - CROSSHAIR_OFFSET - BAND_HEIGHT * this.scale) / 2F;

            // Scale the band via the pose stack; the blit below captures the current pose.
            ...
            graphics.blit(RenderPipelines.GUI_TEXTURED, COMPASS_TEXTURE, (int)x, (int)y, direction, v, (int)BAND_WIDTH, (int)BAND_HEIGHT, TEXTURE_SIZE, TEXTURE_SIZE);
        } finally {
            pose.popMatrix();
        }
    }
