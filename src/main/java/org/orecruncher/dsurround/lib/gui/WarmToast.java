...
public class WarmToast implements Toast {
    ...
    @Override
    public void update(@NotNull ToastManager manager, long lastChanged) {
        if (this.changed) {
    ...
    @Override
    public void extractRenderState(@NotNull GuiGraphicsExtractor graphics, @NotNull Font font, long lastChanged) {
        int i = this.width();
        if (i == 160 && this.messageLines.size() <= 1) {
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.profile.sprite, 0, 0, i, this.height());
        } else {
        ...
        if (this.messageLines.isEmpty()) {
            graphics.text(font, this.title, 18, LINE_SPACING, this.profile.titleColor.getValue(), false);
        } else {
            graphics.text(font, this.title, 18, 7, this.profile.titleColor.getValue(), false);

            for (int j = 0; j < this.messageLines.size(); ++j) {
                graphics.text(font, this.messageLines.get(j), 18, 18 + j * LINE_SPACING, this.profile.bodyColor.getValue(), false);
            }
        }
    }

    private void renderBackgroundRow(GuiGraphicsExtractor graphics, int i, int j, int k, int l) {
        int m = j == 0 ? 20 : 5;
        int n = Math.min(60, i - m);
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.profile.sprite, 160, 32, 0, k, 0, j, m, l);

        for (int o = m; o < i - n; o += 64) {
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.profile.sprite, 160, 32, o, k, 32, j, Math.min(64, i - o - n), l);
        }

        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.profile.sprite, 160, 32, i - n, k, 160 - n, j, n, l);
    }
