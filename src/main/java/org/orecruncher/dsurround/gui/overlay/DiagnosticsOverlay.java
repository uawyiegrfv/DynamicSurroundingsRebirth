...
public class DiagnosticsOverlay extends AbstractOverlay {
    ...
    public void render(GuiGraphics graphics, float partialTick) {
        if (this.showHud) {
            ...
            this.drawText(graphics, this.left, true);
            this.drawText(graphics, this.right, false);
            this.rendering.end();
        }
    }
    ...
    private void drawText(GuiGraphics graphics, ObjectArray<FormattedCharSequence> text, boolean left) {
        ...
        for (j = 0; j < text.size(); ++j) {
            ...
            l = left ? 2 : graphics.guiWidth() - 2 - k;
            m = 2 + i * j;
            graphics.fill(l - 1, m - 1, l + k + 1, m + i - 1, BACKGROUND_COLOR);
            graphics.drawString(textRenderer, component, l, m, FOREGROUND_COLOR, false);
        }
    }
}
