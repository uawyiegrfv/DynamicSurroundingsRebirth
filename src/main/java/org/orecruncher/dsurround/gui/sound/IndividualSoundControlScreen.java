...
public class IndividualSoundControlScreen extends Screen {
    ...
    // 26.1: Background extraction is now driven by the framework through
    // extractRenderStateWithTooltipAndSubtitles(). Preserve the old behavior where the
    // keybind-opened screen uses a transparent background instead of the menu background.
    @Override
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        var renderWidth = Mth.clamp(graphics.guiWidth() - 20, 200, SELECTION_WIDTH);
        this.soundConfigList.setRowWidth(renderWidth);

        graphics.centeredText(this.font, this.title, this.width / 2, TOP_OFFSET, ColorPalette.MC_WHITE.getValue());

        this.soundConfigList.extractRenderState(graphics, mouseX, mouseY, partialTicks);
        this.searchField.extractRenderState(graphics, mouseX, mouseY, partialTicks);
        this.save.extractRenderState(graphics, mouseX, mouseY, partialTicks);
        this.cancel.extractRenderState(graphics, mouseX, mouseY, partialTicks);
        ...
        if (this.soundConfigList.isMouseOver(mouseX, mouseY)) {
            ...
            if (entry != null) {
                ...
                graphics.setTooltipForNextFrame(toolTip, mouseX, mouseY + TOOLTIP_Y_OFFSET);
            }
        }
    }
