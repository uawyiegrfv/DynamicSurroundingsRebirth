        ...
        try (ByteBufferBuilder byteBufferBuilder = ByteBufferBuilder.exactlySized(quadCount * DefaultVertexFormat.PARTICLE.getVertexSize() * 4)) {
            ...
            for (int gridZ = playerZ - range; gridZ <= playerZ + range; gridZ++) {
                for (int gridX = playerX - range; gridX <= playerX + range; gridX++) {
                    ...
                    bufferBuilder.addVertex(x0, y0, z0).setUv(0F, v0).setColor(argb).setLight(light);
                    bufferBuilder.addVertex(x1, y0, z1).setUv(1F, v0).setColor(argb).setLight(light);
                    bufferBuilder.addVertex(x1, y1, z1).setUv(1F, v1).setColor(argb).setLight(light);
                    bufferBuilder.addVertex(x0, y1, z0).setUv(0F, v1).setColor(argb).setLight(light);
                    quads++;
                }
            }
