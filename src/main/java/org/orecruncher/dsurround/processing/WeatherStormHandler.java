...
public class WeatherStormHandler extends AbstractClientHandler {
    ...
    private static final ResourceLocation DUST_CALM = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_calm.png");
    private static final ResourceLocation DUST_LIGHT = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_light.png");
    private static final ResourceLocation DUST_GENTLE = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_gentle.png");
    private static final ResourceLocation DUST_MODERATE = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_moderate.png");
    private static final ResourceLocation DUST_HEAVY = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_heavy.png");
    private static final ResourceLocation DUST_STRONG = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_strong.png");
    private static final ResourceLocation DUST_INTENSE = ResourceLocation.fromNamespaceAndPath(MOD_ID, "textures/environment/dust_intense.png");
    ...
    /**
     * The sandstorm dust veil: vertical quads in a grid around the player, textured
     * with the intensity-graded dust strip and scrolling UVs - the 1.12.2 StormRenderer
     * mechanism. Drawn right after the vanilla weather pass (AFTER_WEATHER stage) so
     * terrain occlusion and translucency behave like rain. Uses the fixed-function
    ...
    private void onAfterWeather(RenderLevelStageEvent event) {
        ...
        for (int gridZ = playerZ - range; gridZ <= playerZ + range; gridZ++) {
            for (int gridX = playerX - range; gridX <= playerX + range; gridX++) {
                ...
                int k2, l2;
                if (nether) {
                    // Nether: surround the player (the nether is a cave - MOTION_BLOCKING
                    // reports the bedrock ceiling, so clamping to surface draws nothing).
                    k2 = playerY - range;
                    l2 = playerY + range;
                } else {
                ...
                buffer.addVertex(x0, y0, z0).setUv((float) d9, v0 + (float) d10).setColor(argb).setLight(light);
                buffer.addVertex(x1, y0, z1).setUv(1F + (float) d9, v0 + (float) d10).setColor(argb).setLight(light);
                buffer.addVertex(x1, y1, z1).setUv(1F + (float) d9, v1 + (float) d10).setColor(argb).setLight(light);
                buffer.addVertex(x0, y1, z0).setUv((float) d9, v1 + (float) d10).setColor(argb).setLight(light);
                quads++;
            }
        }
