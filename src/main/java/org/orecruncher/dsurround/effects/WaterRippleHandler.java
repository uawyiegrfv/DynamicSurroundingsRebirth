...
/**
 * Replaces vanilla rain splash particles with expanding water ripples.
 *
 * 26.1: the old implementation funneled ripples through ParticleRenderCollection (a
 * custom-render particle that bound a standalone texture and drew the group itself).
 * That rendering model is gone - particles now submit quads through the particle
 * render state. Each ripple renders itself, so it is added straight to the engine.
 */
    ...
    public static void createRippleParticle(ClientLevel world, Particle particle, Vec3 position) {
        if (doRipples()) {
            ...
            if (fluidState.isSource() && TAG_LIBRARY.is(FluidTags.WATER_RIPPLES, fluidState)) {
                ...
                //
                // Do NOT reposition it first. setPos(0,0,0) moved the doomed particle to
                // the world origin, where it could be drawn for a frame before
                // ParticleEngine drops it on the next tick - sampling light from unloaded
                // chunks and showing up as a black quad flashing at the origin (reported
                // while fishing in the rain). Marking it removed is sufficient: isAlive()
                // reports false, so it is dropped at the next tick either way.
                if (particle != null) {
                    particle.remove();
                }
            }
