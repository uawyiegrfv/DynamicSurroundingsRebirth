...
public final class AudioTuning {
    ...
    private static volatile float diffractionHfDamping =
    ...
    /**
     * Exponent applied to the diffraction restore when it drives the high-frequency gain.
     * 1.0 = broadband restore (original behaviour); higher damps the restored highs harder than the
     * level, so a source behind a wall comes back audible but dull.
     */
    public static float diffractionHfDamping() {
        return diffractionHfDamping;
    }
    ...
    public static String set(final String key, final String value) {
        switch (key) {
            case "diffractionHfDamping": {
                final float v = clamp(parseFloat(key, value), 1F, 4F);
                final float old = diffractionHfDamping;
                diffractionHfDamping = v;
            ...
            default:
                throw new IllegalArgumentException("Unknown key '" + key + "'. Try: "
                        + "diffractionHfDamping, occlusionSegments, occlusionFanRays, evaluateOnSoundThread");
        }
    }

    /** Restores every value from the config file. */
    public static String reset() {
        diffractionHfDamping = Math.max(1F, (float) CONFIG.diffractionHfDamping);
    ...
    public static String describeAll() {
        return String.format(
                "  diffractionHfDamping   = %.2f   (config: enhancedSounds.diffractionHfDamping, 1.0-4.0; 1.0 = original broadband restore)%n"
                        + "  occlusionSegments      = %d   (config: enhancedSounds.occlusionSegments, 1-32)%n"
                        + "  occlusionFanRays       = %d   (config: enhancedSounds.occlusionFanRays; rounded to 1 + 4n, now %d rays)%n"
                ...
                diffractionHfDamping, occlusionSegments, occlusionFanRays(), occlusionFanRays(),
                evaluateOnSoundThread);
    }
