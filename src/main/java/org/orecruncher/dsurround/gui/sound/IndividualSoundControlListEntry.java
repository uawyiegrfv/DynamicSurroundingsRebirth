...
public class IndividualSoundControlListEntry extends ContainerObjectSelectionList.Entry<IndividualSoundControlListEntry> implements AutoCloseable {
    ...
    @Override
    public void extractContent(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float partialTicks) {
        // 26.1: the entry's position/size is stored on the LayoutElement by the list.
        final var font = GameUtils.getTextRenderer();
        final int rowLeft = this.getX();
        final int rowTop = this.getY();
        final int rowWidth = this.getWidth();
        final int rowHeight = this.getHeight();
        final int labelY = rowTop + (rowHeight - font.lineHeight) / 2;

        // Resize the label so the controls on the right remain visible
        int fixedWidth = this.cullButton.getWidth() + this.blockButton.getWidth() + this.volume.getWidth() + 4 * CONTROL_SPACING;
        if (this.playButton != null)
            fixedWidth += this.playButton.getWidth() + CONTROL_SPACING;
        int labelWidth = rowWidth - fixedWidth;
        if (labelWidth < 100)
            labelWidth = 100;
        ...
        for (final AbstractWidget w : this.children)
            w.extractRenderState(graphics, mouseX, mouseY, partialTicks);
    }
