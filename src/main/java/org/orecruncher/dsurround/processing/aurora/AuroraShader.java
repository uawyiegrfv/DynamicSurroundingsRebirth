...
/**
 * Shader-driven aurora renderer. Clean-room rewrite for 26.1 (MIT), adapted to
 * the 1.20.1 ShaderInstance/RenderType path.
 *
...
public final class AuroraShader extends AuroraBase {
    ...
    private void renderRibbon(final PoseStack poseStack, final VertexConsumer consumer,
        ...
        for (int i = 0; i < array.length - 1; i++) {
            ...
            consumer.vertex(pose, x0, 0.0F, z0).uv(u0, 0.0F).color(r, g, b, alphaByte).endVertex();
            consumer.vertex(pose, x0, 1.0F, z0).uv(u0, 1.0F).color(r, g, b, alphaByte).endVertex();
            consumer.vertex(pose, x1, 1.0F, z1).uv(u1, 1.0F).color(r, g, b, alphaByte).endVertex();
            consumer.vertex(pose, x1, 0.0F, z1).uv(u1, 0.0F).color(r, g, b, alphaByte).endVertex();
        }
    }
