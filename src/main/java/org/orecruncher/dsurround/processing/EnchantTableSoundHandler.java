 ...
 * ticker (bookAnimationTick) opens the book when a player comes within 3
 * blocks and randomly turns the book's page target while a player is nearby.
 * Detection rides on that ticker via MixinEnchantingTableBook, reading the
 * public animation fields:
 *
 * - book opening: oOpen == 0 && open > 0 (first tick of the opening ramp)
 * - book closing: oOpen >= 1 && open < oOpen (first tick of the close ramp)
 * - page turn:    flipT changed while the book is fully open
 *
 * The opening/closing use the DS heavy page flip (the close pitched down);
 * the page turns use the vanilla book page rustle so frequent flips stay
 * subtle.
 */
public final class EnchantTableSoundHandler {

    private static final Identifier BOOK_OPEN = Identifier.fromNamespaceAndPath(
            Constants.MOD_ID, "enchant/book_open");
    private static final Identifier PAGE_TURN = Identifier.fromNamespaceAndPath(
            Constants.MOD_ID, "enchant/page_turn");
    private static final Identifier BOOK_CLOSE = Identifier.fromNamespaceAndPath(
    ...
    private static void play(final Identifier factoryId, final BlockPos pos) {
