    ...
    public void onCollect(CollectDiagnosticsEvent event) {
        ...
        for (var category : SoundSource.values()) {
            ...
            if (Float.compare(volumeSettings, 0F) == 0) {
                var text = Component.literal("%s is OFF".formatted(category.name())).withStyle(st -> st.withColor(net.minecraft.network.chat.TextColor.fromRgb(ColorPalette.RED.getValue())));
                panelText.add(text);
            }
        }
        ...
        // 26.1: getDebugString() was renamed to getChannelDebugString()
