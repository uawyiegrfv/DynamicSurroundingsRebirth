...
public abstract class CuboidScanner extends Scanner {
    ...
    protected BoundingBox activeCuboid;
    ...
    protected BoundingBox getVolumeFor(final BlockPos pos) {
    ...
    public void tick() {
        ...
        if (this.locus.isOutOfHeightLimit(playerPos.getY())) {
            ...
            // The player moved.
            final BoundingBox oldVolume = this.activeCuboid != null ? this.activeCuboid : getVolumeFor(this.lastPos);
            final BoundingBox newVolume = getVolumeFor(playerPos);
            final BoundingBox intersect = Cuboid.intersection(oldVolume, newVolume);
    ...
    protected void updateScan(final BoundingBox newVolume, final BoundingBox oldVolume,
                              final BoundingBox intersect) {
    ...
    public void onBlockUpdates(Collection<BlockPos> positions) {
        if (!positions.isEmpty() && this.activeCuboid != null) {
            ...
            positions.stream()
                    .filter(p -> this.activeCuboid.isInside(p))
                    .forEach(p -> {
