...
public record SoundMapping(Identifier soundEvent, ObjectArray<Mapping> rules) {
     ...
     * it. Vanilla sandstone resolves to {@code concrete} <em>plus</em> a sand accent; a modded
     ...
     */
    private List<Identifier> accentsFor(final Identifier factory,
                                              final List<Identifier> fallback) {
        for (var rule : this.rules)
    ...
    public record Mapping(ObjectArray<IMatcher<BlockState>> blocks, Identifier factory, List<Identifier> accent) {
         ...
         */
        public record MatchResult(Identifier factory, List<Identifier> accent,
                                 boolean explicit) {}
