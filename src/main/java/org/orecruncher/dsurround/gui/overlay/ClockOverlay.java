...
public class ClockOverlay extends AbstractOverlay {
    ...
    public void tick(Minecraft client) {
        ...
        if (this.config.compassAndClockOptions.enableClock && GameUtils.isInGame()) {
            ...
            // Calculate the color this tick. 26.1: getTimeOfDay() was replaced by the
            // overworld clock time; keep the same day-phase math using the clock ticks.
            var world = player.level();
            // 0 is noon, 180 is midnight. Need to normalize so that midnight 0.
            var angleDegrees = (world.getOverworldClockTime() % 24000) * 360F / 24000F + 180;
            // Wrap
            if (angleDegrees >= 360)
                angleDegrees -= 360;
    ...
    public void render(GuiGraphicsExtractor graphics, float partialTick) {
        ...
        var y = graphics.guiHeight() - BOTTOM_OFFSET;

        if (this.clockDisplay.size() == 2)
            y -= textRender.lineHeight / 2;

        // 26.1: renderTooltipBackground -> extractTooltipBackground (style is optional/nullable)
        TooltipRenderUtil.extractTooltipBackground(graphics, x - this.renderWidth / 2, y, this.renderWidth, this.renderHeight, null);
        graphics.centeredText(textRender, this.clockDisplay.get(0), x, y, this.color);
        if (this.clockDisplay.size() == 2)
            graphics.centeredText(textRender, this.clockDisplay.get(1), x, y + textRender.lineHeight, this.color);
    }
}
